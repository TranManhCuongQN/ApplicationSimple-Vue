...
Vue.createApp({
  ...
  // theo dõi một cái data thì mình sẽ dùng watch, không dùng computed
  watch: {
    "gameData.badSelect": function (newVal, oldVal) {
      if (newVal <= 0) {
        this.gameResult = {
          win: false,
          lose: true,
    ...
    "gameData.selectDeck": function (newVal, oldVal) {
      console.log(newVal);
      const deck = parseInt(newVal);
  ...
  computed: {
    ...
    // thẻ đã mở
    uncoveredCards() {
      let uncoveredCards = [...this.selectCard, ...this.pairedCard];
  ...
  methods: {
    ...
    handleResetGame(deck) {
      const pokemonCardList = this.createPokemonDeck(deck);
    ...
    createPokemonDeck(deck) {
      const pokemonCardList = [];
      for (let i = 0; i < deck; i++) {
        const numberRange__00_99 = getRamdomInt(0, 99);
        const pokemonCard = dataPokemon[numberRange__00_99];
    ...
    createPairPokemon(pokemonCardList) {
      ...
      pokemonCardList.map((pokemonCard) => {
        // tránh tham chiếu nên dùng  spread operator
